<x-app-layout>
    ...
    <style>
        ...
        /* FIX 1: Ensure Modal is ON TOP of the sidebar */
        .swal2-container {
            z-index: 999999 !important; 
        }

        /* FIX 2: Stop Sidebar from jumping/breaking when modal opens */
        body.swal2-shown {
    ...
    <div x-data="{ 
        selectedIds: [],
        allIds: {{ json_encode($capitals->pluck('id')) }},
        ...
        bulkForceDelete() {
            ...
            Swal.fire({
                title: '{{ __('capital.warning_perm_delete') }}',
                text: '{{ __('capital.cant_undone') }}', 
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ef4444',
                cancelButtonColor: '#64748b',
                confirmButtonText: '{{ __('capital.yes_delete') }}',
                cancelButtonText: '{{ __('capital.cancel') }}',
                scrollbarPadding: false, /* FIX: Prevents sidebar jump/gap */
                heightAuto: false,       /* FIX: Prevents layout shift */
                background: '#fff',
                borderRadius: '1rem',
                customClass: {
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex items-center gap-3">
                ...
                <h3 x-show="selectedIds.length === 0" class="text-xl font-black text-slate-800 tracking-tight">{{ __('capital.trash') }}</h3>
                ...
                <div x-show="selectedIds.length > 0" x-transition class="flex items-center gap-2" style="display: none;">
                    ...
                    {{-- Cancel --}}
                    <button @click="selectedIds = []" class="text-slate-400 hover:text-slate-600 text-xs underline px-2">{{ __('capital.cancel') }}</button>
                </div>
            </div>
            
            {{-- Back Button --}}
            <a href="{{ route('capitals.index') }}" class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 hover:text-indigo-600 transition shadow-sm font-medium">
            ...
            <table class="w-full text-sm text-left rtl:text-right text-slate-500">
                <thead class="text-xs text-slate-700 uppercase bg-red-50/50 border-b border-red-100">
                    <tr>
                        ...
                        <th class="px-6 py-3 font-bold">{{ __('capital.owner') }}</th>
                        <th class="px-6 py-3 font-bold">{{ __('capital.currency') }}</th>
                        <th class="px-6 py-3 font-bold text-right">{{ __('capital.amount') }}</th>
                        <th class="px-6 py-3 font-bold text-center">{{ __('capital.deleted_by') }}</th>
                        <th class="px-6 py-3 font-bold text-center">{{ __('capital.deleted_at') }}</th>
                        <th class="px-6 py-3 font-bold text-center w-32">{{ __('capital.action') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($capitals as $capital)
                    <tr class="bg-white hover:bg-red-50/30 transition-colors group" :class="selectedIds.includes({{ $capital->id }}) ? 'bg-red-50/60' : ''">
                        
                        {{-- Checkbox --}}
                        <td class="px-4 py-4 text-center">
                            <input type="checkbox" :value="{{ $capital->id }}" x-model="selectedIds" class="select-checkbox">
                        </td>
                        ...
                        {{-- Name --}}
                        <td class="px-6 py-4">
                            <span class="font-bold text-slate-800 text-sm">{{ $capital->owner->name ?? '-' }}</span>
                        </td>
                        
                        {{-- Currency --}}
                        <td class="px-6 py-4 text-slate-600 font-medium">
                            {{ $capital->currency->currency_type ?? '-' }}
                        </td>
                        
                        {{-- Balance --}}
                        <td class="px-6 py-4 text-right text-emerald-600 font-bold mx-2">{{ number_format($capital->amount, 0) }}</td>
                            ...
                            @if($capital->creator) 
                                <div class="flex items-center justify-center gap-2">
                                    <div class="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-600 uppercase border border-slate-300">
                                        {{ substr($capital->creator->name, 0, 1) }}
                                    </div>
                                    <span class="text-xs font-bold text-slate-700">{{ $capital->creator->name }}</span>
                                </div>
                            @else
                        ...
                        <td class="px-6 py-4 text-center text-xs text-red-500 font-medium">
                            <div class="bg-red-50 px-2 py-1 rounded border border-red-100 inline-block">
                                {{ $capital->deleted_at->format('Y-m-d H:i') }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                {{-- Restore Button --}}
                                <form action="{{ route('capitals.restore', $capital->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg transition-colors shadow-sm" title="{{ __('capital.restore') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                    </button>
                                </form>

                                {{-- Force Delete Button --}}
                                <button type="button" onclick="confirmForceDelete({{ $capital->id }})" class="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors shadow-sm" title="{{ __('capital.perm_delete') }}">
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center text-slate-400">
                            <div class="flex flex-col items-center justify-center gap-3">
                                ...
                                <p class="text-sm font-medium">{{ __('capital.trash_empty') }}</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <form id="bulk-restore-form" action="{{ route('capitals.bulk-restore') }}" method="POST" class="hidden">@csrf <input type="hidden" name="ids" id="bulk-restore-ids"></form>
    <form id="bulk-force-delete-form" action="{{ route('capitals.bulk-force-delete') }}" method="POST" class="hidden">@csrf @method('DELETE') <input type="hidden" name="ids" id="bulk-force-delete-ids"></form>
    ...
    <script>
        function confirmForceDelete(id) {
            ...
            form.action = "{{ route('capitals.forceDelete', ':id') }}".replace(':id', id);
            ...
            Swal.fire({
                ...
                confirmButtonText: '{{ __('capital.yes_delete') }}',
                cancelButtonText: '{{ __('capital.cancel') }}',
                scrollbarPadding: false, /* FIX: No layout shift */
                heightAuto: false,
                background: '#fff',
                borderRadius: '1rem',
                customClass: {
