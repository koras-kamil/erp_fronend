<x-app-layout>
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex items-center gap-3">
                ...
                <h3 x-show="selectedIds.length === 0" class="text-xl font-black text-slate-800 tracking-tight">{{ __('currency.trash') }}</h3>

                {{-- BULK ACTIONS BAR (Visible when items selected) --}}
                <div x-show="selectedIds.length > 0" x-transition class="flex items-center gap-2">
                    <span class="text-xs font-bold text-red-600 bg-red-50 px-2 py-1 rounded border border-red-100"><span x-text="selectedIds.length"></span> {{ __('currency.selected') }}</span>
                    ...
                    {{-- Cancel --}}
                    <button @click="selectedIds = []" class="text-slate-400 hover:text-slate-600 text-xs underline px-2">{{ __('currency.cancel') }}</button>
                </div>
            </div>
        ...
        <div class="relative overflow-x-auto bg-white shadow-sm rounded-xl border border-slate-200 mx-4">
            <table class="w-full text-sm text-left rtl:text-right text-slate-500">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="bg-white hover:bg-red-50/30 transition-colors group" :class="selectedIds.includes({{ $currency->id }}) ? 'bg-red-50/60' : ''">
    ...
    <form id="bulk-restore-form" action="{{ route('currency.bulk-restore') }}" method="POST" class="hidden">@csrf <input type="hidden" name="ids" id="bulk-restore-ids"></form>
    <form id="bulk-force-delete-form" action="{{ route('currency.bulk-force-delete') }}" method="POST" class="hidden">@csrf @method('DELETE') <input type="hidden" name="ids" id="bulk-force-delete-ids"></form>
