    ...
    <div x-data="accountantManager()" x-init="init()" class="py-6 w-full min-w-0 bg-white min-h-screen" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        ...
        <div class="mx-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            <div class="flex items-center gap-3">
                <div class="bg-slate-100 p-1 rounded-lg flex items-center shadow-inner">
                    <span class="px-5 py-2 text-sm font-bold rounded-md bg-white text-emerald-600 shadow-sm transition">
                        {{ app()->getLocale() == 'ku' ? 'پسوڵەی وەرگرتن' : __('accountant.receiving_title') }}
                    </span>
                </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <x-btn type="add" @click="$dispatch('open-receiving-modal')" title="{{ __('Add New') }}" />
            </div>
        </div>
        ...
        <div class="relative w-full overflow-x-auto table-container bg-white shadow-sm rounded-lg border border-slate-200 mx-4 pb-20">
            <table class="w-full text-sm text-left rtl:text-right text-slate-500 whitespace-nowrap border-separate border-spacing-0">
                <thead class="text-xs text-slate-700 uppercase bg-emerald-50 border-b border-emerald-100">
                    <tr>
                        <th class="px-4 py-3 w-[40px] text-center bg-emerald-50 border-b border-emerald-100 sticky left-0 z-10">
                            <input type="checkbox" @click="toggleAllSelection()" :checked="allSelected" class="select-checkbox bg-white">
                        </th>

                        {{-- 🟢 FIXED: Draggable Columns with STRICT min-width to prevent squishing --}}
                        <template x-for="(col, index) in columns" :key="col.field">
                            <th x-show="col.visible" 
                                class="py-2 relative h-12 transition-colors duration-200 border-r border-transparent select-none group border-b border-emerald-100 bg-emerald-50"
                                ...
                                <div class="th-container" :class="{ 'search-active': openFilter === col.field }">
                                    {{-- Title --}}
                                    <div class="th-title">
                                      <div @click="sortBy(col.field)" class="flex items-center justify-center gap-1 cursor-pointer flex-1 h-full hover:text-emerald-600 transition-colors overflow-hidden">
    <span x-text="col.label" class="truncate font-bold tracking-wide text-[11px] text-center"></span>
    <svg class="w-3 h-3 text-emerald-500 transition-transform shrink-0" :class="sortCol === col.field && !sortAsc ? 'rotate-180' : ''" x-show="sortCol === col.field" fill="currentColor" viewBox="0 0 20 20"><path d="M5 10l5-5 5 5H5z"/></svg>
</div>
                                        <button type="button" @click.stop="openFilter = col.field; setTimeout(() => $refs['input-'+col.field].focus(), 100)" class="p-1 rounded-md text-slate-400 hover:text-emerald-600 hover:bg-slate-100 transition shrink-0" :class="filters[col.field] ? 'text-emerald-600' : ''">
                        ...
                        <th class="px-4 py-3 w-[80px] text-center print:hidden bg-emerald-50 border-b border-emerald-100">{{ __('accountant.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    <template x-for="(trx, rowIndex) in filteredTransactions" :key="trx.id">
                        <tr class="hover:bg-slate-50 transition-colors group" :class="selectedIds.includes(trx.id) ? 'bg-emerald-50/10' : ''">
                            
                            <td class="px-4 py-2 text-center sticky left-0 bg-white group-hover:bg-slate-50 transition-colors"><input type="checkbox" :value="trx.id" x-model="selectedIds" class="select-checkbox"></td>
                         ...
                         <template x-for="col in columns" :key="col.field">
                                <td x-show="col.visible" class="p-2 align-middle">
                                    ...
                                    <template x-if="col.field === 'account_id'">
                                        <div class="flex justify-center items-center w-full px-2">
                                            <a :href="'/accountant/statement?account_id=' + trx.account_id" class="flex items-center gap-2 group/link hover:bg-emerald-50/80 p-1.5 rounded-lg transition-colors cursor-pointer w-fit text-right" title="View Statement">
                                                ...
                                                <div class="flex flex-col leading-none text-right">
                                                    <span class="font-bold text-emerald-600 text-xs truncate max-w-[160px] group-hover/link:text-emerald-800 transition-colors" x-text="trx.account?.name"></span>
                                                    <span class="text-[10px] text-slate-400 font-mono mt-0.5 group-hover/link:text-emerald-500 transition-colors" x-text="trx.account?.code"></span>
                                                </div>
                                            </a>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="col.field === 'amount'">
                                        <div class="flex justify-center items-center w-full px-3">
                                            <span class="font-bold text-emerald-600 text-sm" x-text="formatMoney(trx.amount)"></span>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="col.field === 'discount'">
                                        <div class="flex justify-center items-center w-full px-3" x-show="trx.discount > 0">
                                            <span class="text-rose-500 text-sm font-bold" x-text="formatMoney(trx.discount)"></span>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="col.field === 'cashbox_id'"><div class="flex justify-center items-center w-full px-3"><span class="text-xs font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100" x-text="trx.cashbox?.name || '-'"></span></div></template>
                            ...
                            {{-- 🟢 FIXED: Actions are now ALWAYS visible, completely removed opacity hide/show logic --}}
                            <td class="px-3 py-2 text-center print:hidden bg-white group-hover:bg-slate-50 transition-colors">
                                <div class="flex items-center justify-center gap-1.5">
                                    <x-btn type="edit" @click="$dispatch('open-receiving-modal', trx)" title="{{ __('accountant.edit') }}" />
                                    <form :action="`/accountant/receiving/${trx.id}`" method="POST" class="m-0 p-0 inline">
                                        @csrf @method('DELETE')
    ...
    <script>
        function accountantManager() {
            return {
                ...
                // 🟢 FIXED: Adjusted default widths to be much wider so all names auto-fit perfectly!
                columns: [
                    ...
                    {{-- 🟢 FIXED: Changed to total_invoice --}}
                ...
                init() { 
                    // 🟢 Bumped to v6 so the new column widths and language updates apply instantly!
                    const savedCols = localStorage.getItem('receiving_cols_v6');
                    if (savedCols) this.columns = JSON.parse(savedCols);
                    this.columns.forEach(c => this.filters[c.field] = '');
                },
                
                resetLayout() {
                    localStorage.removeItem('receiving_cols_v6');
                    location.reload();
                },
                ...
                initResize(e, col) { 
                    ...
                    const onMouseMove = (moveEvent) => { 
                        ...
                        // 🟢 FIXED: Prevents squishing the column down too small (minimum 90px)
                ...
                saveState() { localStorage.setItem('receiving_cols_v6', JSON.stringify(this.columns)); },
                ...
                get processedTransactions() {
                    return this.transactions.map(trx => {
                        ...
                        // 🟢 Total Invoice = Cash Amount + Discount
