<x-app-layout>
    ...
    <div x-data="entriesManager()" x-init="initData()" class="py-6 w-full min-w-0" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        ...
        <div class="mx-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex items-center gap-3">
                <h2 class="text-xl font-bold text-slate-800">{{ __('accounting_entry.title') }}</h2>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <select x-model="dateFilter" @change="fetchData()" class="w-auto inline-block bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 py-2 ltr:pl-3 ltr:pr-10 rtl:pr-3 rtl:pl-10 shadow-sm transition-colors cursor-pointer hover:bg-slate-50 mr-2 ml-2">
                    <option value="all">{{ __('accounting_entry.all_time') }}</option>
                    <option value="today">{{ __('accounting_entry.today') }}</option>
                    <option value="yesterday">{{ __('accounting_entry.yesterday') }}</option>
                    <option value="this_month">{{ __('accounting_entry.this_month') }}</option>
                    <option value="last_month">{{ __('accounting_entry.last_month') }}</option>
                    <option value="this_year">{{ __('accounting_entry.this_year') }}</option>
                    <option value="last_year">{{ __('accounting_entry.last_year') }}</option>
                </select>
                ...
                <div x-show="selectedIds.length > 0" x-transition class="flex items-center gap-2 bg-red-50 px-2 py-1 rounded-lg border border-red-100 mr-2 ml-2">
                    <span class="text-xs font-bold text-red-600 px-2"><span x-text="selectedIds.length"></span> {{ __('accounting_entry.selected') }}</span>
                ...
                <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                    <x-btn type="columns" @click="open = !open" title="{{ __('accounting_entry.columns') }}" />
                    <div x-show="open" @click.stop class="absolute top-full mt-3 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 z-50 p-3 ltr:right-0 rtl:left-0" style="display:none;">
                        <div class="flex justify-between items-center px-2 py-1 mb-2 border-b border-slate-100 pb-2">
                            <span class="text-[10px] font-bold text-slate-400 uppercase">{{ __('accounting_entry.columns') }}</span>
                            <button @click="resetLayout(); open = false;" class="text-[10px] text-blue-500 hover:underline cursor-pointer">{{ __('accounting_entry.reset_layout') }}</button>
                        </div>
                ...
                <x-btn type="print" href="#" target="_blank" title="{{ __('accounting_entry.print') }}" />
                <x-btn type="add" href="{{ route('accountant.accounting_entries.create') }}" title="{{ __('accounting_entry.new_entry') }}" />
            </div>
        </div>
        ...
        <div class="relative w-full overflow-x-auto table-container bg-white shadow-sm rounded-lg border border-slate-200 mx-4 pb-20">
            ...
            <table class="w-full text-sm text-center rtl:text-center text-slate-500 whitespace-nowrap border-separate border-spacing-0">
                <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100 sticky top-0 z-20">
                    <tr>
                        <th class="px-4 py-3 w-[40px] text-center bg-slate-50/95 border-b border-blue-100 border-x border-slate-200/50"><input type="checkbox" @click="toggleAllSelection()" :checked="data.length > 0 && selectedIds.length === data.length" class="select-checkbox bg-white"></th>
                        <template x-for="(col, index) in columns" :key="col.field">
                            <th x-show="col.visible" 
                                class="px-4 py-2 relative h-12 text-center transition-colors duration-200 border-x border-slate-200/50 select-none group border-b border-blue-100 bg-slate-50/95" 
                                ...
                                <div class="th-container" :class="{ 'search-active': openFilter === col.field }">
                                    ...
                                    <div x-show="col.searchable !== false" class="th-search" @click.stop>
                                        ...
                                        <input type="text" :x-ref="'input-'+col.field" x-model="filters[col.field]" @input.debounce.400ms="fetchData()" @keydown.escape="openFilter = null; filters[col.field] = ''; fetchData();" class="header-search-input w-full" placeholder="{{ __('accounting_entry.search') }}">
                        ...
                        <th class="px-4 py-3 w-[100px] text-center print:hidden bg-blue-50/50 border-b border-blue-100 sticky right-0 z-20 border-x border-slate-200/50">{{ __('accounting_entry.actions') }}</th>
                    </tr>
                </thead>

                <tbody class="divide-y divide-slate-100 bg-white">
                    <template x-for="(entry, index) in data" :key="entry.id">
                        <tr class="transition-colors group hover:bg-slate-100" :class="selectedIds.includes(entry.id) ? 'bg-indigo-50/20' : (index % 2 === 0 ? 'bg-white' : 'bg-slate-50')">
                            
                            <td class="px-4 py-2 text-center align-middle border-x border-slate-200/50"><input type="checkbox" :value="entry.id" x-model="selectedIds" class="select-checkbox"></td>
                                ...
                                <td x-show="col.visible" class="p-1 text-center border-x border-slate-200/50" :class="col.class">
                                    ...
                                    <template x-if="col.field === 'attachment'">
                                        <div class="flex items-center justify-center">
                                            <template x-if="entry.attachment_url">
                                                <a :href="entry.attachment_url" target="_blank" class="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded-lg transition-colors" title="View Attachment">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                                </a>
                                            </template>
                                            <template x-if="!entry.attachment_url"><span class="text-slate-300">-</span></template>
                                        </div>
                                    </template>

                                    {{-- 2. Type Badge (جۆری پسوڵە) --}}
                                    <template x-if="col.field === 'type'">
                                        <div class="flex justify-center">
                                            <span x-show="entry.type === 'cash_in'" class="inline-flex items-center gap-1 bg-emerald-100 text-emerald-700 text-[11px] font-bold px-2.5 py-1 rounded-md">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                                                {{ __('accounting_entry.cash_in') }}
                                            </span>
                                            <span x-show="entry.type === 'cash_out'" class="inline-flex items-center gap-1 bg-rose-100 text-rose-700 text-[11px] font-bold px-2.5 py-1 rounded-md">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                                                {{ __('accounting_entry.cash_out') }}
                                            </span>
                                        </div>
                                    </template>
                                    ...
                                    <template x-if="col.field === 'amount'">
                                        <div class="text-center font-bold text-base" :class="entry.type === 'cash_in' ? 'text-emerald-600' : 'text-rose-600'" x-text="Number(entry.amount) > 0 ? Number(entry.amount).toLocaleString() : '-'"></div>
                                    </template>
                                    <template x-if="col.field === 'exchange_rate'"><div class="text-center text-xs text-slate-500" x-text="Number(entry.exchange_rate).toLocaleString()"></div></template>

                                    {{-- 4. Row Counter & Bold Text Fields --}}
                                    ...
                                    <template x-if="col.field === 'voucher_number'"><div class="font-bold text-slate-700" x-text="entry.voucher_number"></div></template>
                                    <template x-if="col.field === 'manual_voucher'"><div class="font-medium text-slate-600" x-text="entry.manual_voucher"></div></template>
                                    <template x-if="col.field === 'entry_datetime'"><div class="font-medium text-slate-600 text-xs" x-text="entry.entry_datetime" dir="ltr"></div></template>
                                    
                                    <template x-if="!['id','attachment','amount','exchange_rate','voucher_number','manual_voucher','entry_datetime','type'].includes(col.field)">
                                        <div class="px-3 text-xs text-slate-600 truncate text-center" x-text="entry[col.field] || '-'"></div>
                                    </template>
                                </td>
                            </template>
                            ...
                            <td class="px-3 py-2 text-center print:hidden sticky-action group-hover:bg-slate-100 transition-colors border-x border-slate-200/50">
                                <div class="flex items-center justify-center gap-2">
                                    <button @click="deleteRow(entry.delete_url)" class="p-1.5 text-red-600 hover:bg-red-100 rounded-lg transition" title="Delete">
                    ...
                    <tr x-show="!loading && data.length === 0" x-transition.opacity class="bg-white"><td :colspan="Object.keys(columns).length + 2" class="py-10 text-center border-b border-slate-50"><span class="text-slate-400 font-medium mt-4">{{ __('accounting_entry.no_data_found') }}</span></td></tr>
                </tbody>
            </table>
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('entriesManager', () => {
                
                const defaultColumns = [
                    { field: 'id', label: "{{ __('accounting_entry.id') }}", visible: true, width: 50, searchable: false },
                    { field: 'manual_voucher', label: "{{ __('accounting_entry.manual_voucher') }}", visible: true, width: 110, searchable: true },
                    { field: 'entry_datetime', label: "{{ __('accounting_entry.entry_datetime') }}", visible: true, width: 130, searchable: true },
                    { field: 'voucher_number', label: "{{ __('accounting_entry.voucher_number') }}", visible: true, width: 100, searchable: true },
                    { field: 'type', label: "{{ __('accounting_entry.type') }}", visible: true, width: 120, searchable: true },
                    { field: 'amount', label: "{{ __('accounting_entry.amount') }}", visible: true, width: 120, searchable: true },
                    { field: 'currency_name', label: "{{ __('accounting_entry.currency_name') }}", visible: true, width: 90, searchable: true },
                    { field: 'exchange_rate', label: "{{ __('accounting_entry.exchange_rate') }}", visible: true, width: 90, searchable: true },
                    { field: 'cashbox_name', label: "{{ __('accounting_entry.cashbox_name') }}", visible: true, width: 130, searchable: true },
                    { field: 'user_name', label: "{{ __('accounting_entry.user_name') }}", visible: true, width: 110, searchable: true },
                    { field: 'note', label: "{{ __('accounting_entry.note') }}", visible: true, width: 160, searchable: true },
                    { field: 'attachment', label: "{{ __('accounting_entry.attachment') }}", visible: true, width: 80, searchable: false },
                ];
                ...
                return {
                    data: {!! json_encode($entries->items(), JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) !!},
                    pagination: {!! json_encode($entries, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) !!},
                    ...
                    params: { sort: 'entry_datetime', direction: 'desc', page: 1 },

                    initData() { 
                        const saved = localStorage.getItem('acc_entries_table_v2');
                        if(saved) {
                    ...
                    saveState() { localStorage.setItem('acc_entries_table_v2', JSON.stringify(this.columns)); },
                    resetLayout() { localStorage.removeItem('acc_entries_table_v2'); location.reload(); },
                    ...
                    deleteRow(url) {
                        ...
                        form.action = url;
                        if (confirm("{{ __('accounting_entry.are_you_sure_delete') }}")) form.submit();
                    },
                    
                    fetchData(pageUrl = null) { 
                        this.loading = true;
                        let targetUrl = new URL("{{ route('accountant.accounting_entries.index') }}", window.location.origin); 
