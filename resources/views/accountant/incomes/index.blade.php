<x-app-layout>
    ...
    <div x-data="incomesManager()" x-init="initData()" class="py-6 w-full min-w-0" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        ...
        <div class="mx-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            ...
            <div class="flex items-center gap-3">
                <h2 class="text-xl font-bold text-slate-800">{{ __('income.incomes_transactions') }}</h2>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <select x-model="dateFilter" @change="fetchData()" class="w-auto inline-block bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 py-2 ltr:pl-3 ltr:pr-10 rtl:pr-3 rtl:pl-10 shadow-sm transition-colors cursor-pointer hover:bg-slate-50 mr-2 ml-2">
                    <option value="all">{{ __('income.all_time') }}</option>
                    <option value="today">{{ __('income.today') }}</option>
                    <option value="yesterday">{{ __('income.yesterday') }}</option>
                    <option value="this_month">{{ __('income.this_month') }}</option>
                    <option value="last_month">{{ __('income.last_month') }}</option>
                    <option value="this_year">{{ __('income.this_year') }}</option>
                    <option value="last_year">{{ __('income.last_year') }}</option>
                </select>
                ...
                <div x-show="selectedIds.length > 0" x-transition class="flex items-center gap-2 bg-red-50 px-2 py-1 rounded-lg border border-red-100 mr-2 ml-2">
                    <span class="text-xs font-bold text-red-600 px-2"><span x-text="selectedIds.length"></span> {{ __('income.selected') }}</span>
                    <x-btn type="bulk-delete" @click="bulkDelete()">{{ __('income.delete_selected') }}</x-btn>
                ...
                <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                    <x-btn type="columns" @click="open = !open" title="{{ __('income.columns') }}" />
                    <div x-show="open" @click.stop class="absolute top-full mt-3 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 z-50 p-3 ltr:right-0 rtl:left-0" style="display:none;">
                        <div class="flex justify-between items-center px-2 py-1 mb-2 border-b border-slate-100 pb-2">
                            <span class="text-[10px] font-bold text-slate-400 uppercase">{{ __('income.columns') }}</span>
                            <button @click="resetLayout(); open = false;" class="text-[10px] text-blue-500 hover:underline cursor-pointer">{{ __('income.reset_layout') }}</button>
                        </div>
                ...
                <x-btn type="print" href="#" target="_blank" title="{{ __('income.print') }}" />
                <x-btn type="add" href="{{ route('accountant.incomes.create') }}" title="{{ __('income.new_income') }}" />
            </div>
        </div>
            ...
            <table class="w-full text-sm text-center rtl:text-center text-slate-500 whitespace-nowrap border-separate border-spacing-0">
                <thead class="text-xs text-slate-700 uppercase bg-blue-50/50 border-b border-blue-100 sticky top-0 z-20">
                    <tr>
                            ...
                            <th x-show="col.visible" 
                                ...
                                <div class="th-container" :class="{ 'search-active': openFilter === col.field }">
                                    ...
                                    <div x-show="col.searchable !== false" class="th-search" @click.stop>
                                        ...
                                        <input type="text" :x-ref="'input-'+col.field" x-model="filters[col.field]" @input.debounce.400ms="fetchData()" @keydown.escape="openFilter = null; filters[col.field] = ''; fetchData();" class="header-search-input w-full" placeholder="{{ __('income.search') }}">
                        ...
                        <th class="px-4 py-3 w-[100px] text-center print:hidden bg-blue-50/50 border-b border-blue-100 sticky right-0 z-20 border-x border-slate-200/50">{{ __('income.actions') }}</th>
                    </tr>
                </thead>

                <tbody class="divide-y divide-slate-100 bg-white">
                    <template x-for="(inc, index) in data" :key="inc.id">
                        <tr class="transition-colors group hover:bg-slate-100" :class="selectedIds.includes(inc.id) ? 'bg-indigo-50/20' : (index % 2 === 0 ? 'bg-white' : 'bg-slate-50')">
                            
                            <td class="px-4 py-2 text-center align-middle border-x border-slate-200/50"><input type="checkbox" :value="inc.id" x-model="selectedIds" class="select-checkbox"></td>
                                ...
                                <td x-show="col.visible" class="p-1 text-center border-x border-slate-200/50" :class="col.class">
                                    ...
                                    <template x-if="col.field === 'attachment'">
                                        <div class="flex items-center justify-center">
                                            <template x-if="inc.attachment_url">
                                                <a :href="inc.attachment_url" target="_blank" class="p-1.5 text-indigo-500 hover:bg-indigo-50 rounded-lg transition-colors" title="View Attachment">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                                </a>
                                            </template>
                                            <template x-if="!inc.attachment_url"><span class="text-slate-300">-</span></template>
                                        </div>
                                    </template>

                                    {{-- 2. Money Formatting --}}
                                    <template x-if="col.field === 'cash_amount'"><div class="text-center font-bold text-emerald-600" x-text="Number(inc.cash_amount) > 0 ? Number(inc.cash_amount).toLocaleString() : '-'"></div></template>
                                    <template x-if="col.field === 'debt_amount'"><div class="text-center font-bold text-blue-600" x-text="Number(inc.debt_amount) > 0 ? Number(inc.debt_amount).toLocaleString() : '-'"></div></template>
                                    <template x-if="col.field === 'discount'"><div class="text-center font-bold text-orange-500" x-text="Number(inc.discount) > 0 ? Number(inc.discount).toLocaleString() : '-'"></div></template>
                                    <template x-if="col.field === 'exchange_rate'"><div class="text-center text-xs text-slate-500" x-text="Number(inc.exchange_rate).toLocaleString()"></div></template>
                                    ...
                                    <template x-if="col.field === 'voucher_number'"><div class="font-bold text-slate-700" x-text="inc.voucher_number"></div></template>
                                    <template x-if="col.field === 'category_name'"><div class="font-bold text-emerald-600 text-xs" x-text="inc.category_name"></div></template>
                                    ...
                                    <template x-if="!['id','attachment','cash_amount','debt_amount','discount','exchange_rate','voucher_number','category_name'].includes(col.field)">
                                        <div class="px-3 text-xs text-slate-600 truncate text-center" x-text="inc[col.field] || '-'"></div>
                                    </template>
                                </td>
                            </template>
                            ...
                            <td class="px-3 py-2 text-center print:hidden sticky-action group-hover:bg-slate-100 transition-colors border-x border-slate-200/50">
                                <div class="flex items-center justify-center gap-2">
                                    <a :href="inc.edit_url" class="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg transition" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <button @click="deleteRow(inc.delete_url)" class="p-1.5 text-red-600 hover:bg-red-100 rounded-lg transition" title="Delete">
                    ...
                    <tr x-show="!loading && data.length === 0" x-transition.opacity class="bg-white"><td :colspan="Object.keys(columns).length + 2" class="py-10 text-center border-b border-slate-50"><span class="text-slate-400 font-medium mt-4">{{ __('income.no_data_found') }}</span></td></tr>
                </tbody>
            </table>
        </div>
        ...
        <form id="bulk-delete-form" action="{{ route('accountant.incomes.bulk-delete') }}" method="POST" class="hidden">@csrf @method('DELETE')<input type="hidden" name="ids" id="bulk-delete-ids"></form>
    </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('incomesManager', () => {
                ...
                const defaultColumns = [
                    { field: 'id', label: "{{ __('income.id') }}", visible: true, width: 60, searchable: false },
                    { field: 'voucher_number', label: "{{ __('income.voucher_number') }}", visible: true, width: 120, searchable: true },
                    { field: 'income_date', label: "{{ __('income.date') }}", visible: true, width: 110, searchable: true },
                    { field: 'category_name', label: "{{ __('income.category') }}", visible: true, width: 160, searchable: true },
                    { field: 'cash_amount', label: "{{ __('income.cash') }}", visible: true, width: 120, searchable: true },
                    { field: 'debt_amount', label: "{{ __('income.debt') }}", visible: true, width: 120, searchable: true },
                    { field: 'discount', label: "{{ __('income.discount') }}", visible: true, width: 100, searchable: true },
                    { field: 'account_name', label: "{{ __('income.account') }}", visible: true, width: 140, searchable: true },
                    { field: 'currency_name', label: "{{ __('income.currency') }}", visible: true, width: 90, searchable: true },
                    { field: 'exchange_rate', label: "{{ __('income.exchange_rate') }}", visible: true, width: 90, searchable: true },
                    { field: 'cashbox_name', label: "{{ __('income.cashbox') }}", visible: true, width: 130, searchable: true },
                    { field: 'creator_name', label: "{{ __('income.creator') }}", visible: true, width: 120, searchable: true },
                    { field: 'note', label: "{{ __('income.note') }}", visible: true, width: 160, searchable: true },
                    { field: 'manual_voucher', label: "{{ __('income.manual_voucher') }}", visible: true, width: 110, searchable: true },
                    { field: 'attachment', label: "{{ __('income.attachment') }}", visible: true, width: 120, searchable: false },
                ];
                ...
                return {
                    data: {!! json_encode($incomes->items(), JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) !!},
                    pagination: {!! json_encode($incomes, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) !!},
                    ...
                    params: { sort: 'income_date', direction: 'desc', page: 1 },

                    initData() { 
                        const saved = localStorage.getItem('inc_table_v1');
                        if(saved) {
                    ...
                    saveState() { localStorage.setItem('inc_table_v1', JSON.stringify(this.columns)); },
                    resetLayout() { localStorage.removeItem('inc_table_v1'); location.reload(); },
                    ...
                    deleteRow(url) {
                        ...
                        form.action = url;
                        if (confirm("{{ __('income.are_you_sure') }}")) form.submit();
                    },
                    bulkDelete() {
                        if (this.selectedIds.length === 0) return;
                        document.getElementById('bulk-delete-ids').value = JSON.stringify(this.selectedIds);
                        if (confirm("{{ __('income.are_you_sure_bulk') }}")) document.getElementById('bulk-delete-form').submit();
                    },
                    
                    fetchData(pageUrl = null) { 
                        this.loading = true;
                        let targetUrl = new URL("{{ route('accountant.incomes.index') }}", window.location.origin); 
