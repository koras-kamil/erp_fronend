<x-app-layout>
    <div x-data="incomeForm()" class="py-8 w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8" dir="{{ app()->getLocale() == 'ku' ? 'rtl' : 'ltr' }}">
        ...
        <div class="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                    ...
                    {{ __('income.new_income') }}
                </h2>
                <p class="text-sm text-slate-500 mt-1">{{ __('income.new_income_desc') }}</p>
            </div>
            <a href="{{ route('accountant.incomes.index') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50 hover:text-emerald-600 hover:border-emerald-200 transition-all shadow-sm flex items-center gap-2">
        ...
        {{-- Main Form Container --}}
        <form action="{{ route('accountant.incomes.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3 p-6 sm:p-8 flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div class="border border-slate-200 rounded-xl overflow-x-auto overflow-y-visible shadow-sm flex-1 pb-6">
                        <table class="w-full text-sm text-center min-w-[600px]">
                            <thead class="bg-emerald-50/80 text-emerald-900 border-b border-emerald-100">
                                <tr>
                                    <th class="py-3 px-3 w-12 text-xs uppercase tracking-wider font-black">{{ __('income.id') }}</th>
                                    <th class="py-3 px-3 w-1/3 text-xs uppercase tracking-wider font-black">{{ __('income.category') }} *</th>
                                    <th class="py-3 px-3 w-1/4 text-xs uppercase tracking-wider font-black">{{ __('income.amount') }} *</th>
                                    <th class="py-3 px-3 text-xs uppercase tracking-wider font-black">{{ __('income.note') }}</th>
                                ...
                                <template x-for="(item, index) in items" :key="index">
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td class="p-2.5" x-data="{ catSearchOpen: false }">
                                            ...
                                            <div class="relative text-right">
                                                <input type="text" x-model="item.category_name" 
                                                       ...
                                                       class="w-full text-sm font-bold text-slate-700 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 bg-white shadow-sm py-2 px-3 transition-colors"
                                                       :class="!item.category_id && item.category_name !== '' ? 'border-orange-400' : 'border-slate-200'"
                                                       placeholder="{{ __('income.search_category') }}" required>
                                                    ...
                                                    <ul class="max-h-48 overflow-y-auto custom-scrollbar text-right">
                                                        <template x-for="cat in categoriesList.filter(c => c.name.toLowerCase().includes(item.category_name.toLowerCase()))" :key="cat.id">
                                                            <li @click="item.category_id = cat.id; item.category_name = cat.name; catSearchOpen = false;" 
                                                                class="py-2.5 px-4 text-xs hover:bg-emerald-50 cursor-pointer font-bold border-b border-slate-50 last:border-0 text-slate-700" 
                                                                x-text="cat.name">
                                                            </li>
                                                        </template>
                                        ...
                                        <td class="p-2.5">
                                            ...
                                            <input type="number" step="0.01" x-model.number="item.price" class="w-full text-center font-bold text-slate-700 rounded-lg border-slate-200 focus:ring-emerald-500 focus:border-emerald-500 text-sm shadow-sm py-2" placeholder="0" required>
                                        </td>
                                        <td class="p-2.5">
                                            <input type="hidden" :name="'items['+index+'][note]'" :value="item.note">
                                            <input type="text" x-model="item.note" class="w-full text-sm text-right rounded-lg border-slate-200 focus:ring-emerald-500 focus:border-emerald-500 bg-white shadow-sm py-2" placeholder="{{ __('income.note') }}...">
                                        </td>
                                        <td class="p-2.5">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-6 gap-4 pb-4">
                        <button type="button" @click="addItem()" class="flex items-center gap-2 px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold rounded-xl transition-colors text-sm border border-slate-200 shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> {{ __('income.add_row') }}
                        </button>

                        <div class="flex items-center gap-3 bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100 shadow-sm w-full sm:w-auto">
                            
                            {{-- Searchable Currency Dropdown --}}
                            <div class="flex items-center gap-2" x-data="{ searchCurOpen: false, searchCur: '' }">
                                <label class="text-xs font-bold text-emerald-800">{{ __('income.currency') }}</label>
                                <div class="relative">
                                    <input type="hidden" name="currency_id" :value="form.currency_id">
                                    <div @click="searchCurOpen = !searchCurOpen; if(searchCurOpen) $nextTick(() => $refs.searchCurInput.focus())" class="w-28 flex items-center justify-between rounded-xl border border-white shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm font-bold py-2 px-3 bg-white cursor-pointer hover:border-emerald-300 transition-colors">
                                        <span x-text="form.currency_id ? currenciesData[form.currency_id].type : '...'" class="text-emerald-700"></span>
                                    ...
                                    <div x-show="searchCurOpen" @click.outside="searchCurOpen = false" x-transition.opacity class="absolute z-50 w-48 mt-2 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden ltr:left-0 rtl:right-0" style="display: none;">
                                        <div class="p-2 border-b border-slate-100 bg-slate-50">
                                            <input x-ref="searchCurInput" type="text" x-model="searchCur" class="w-full text-xs rounded-lg border-slate-300 py-2 px-3 focus:ring-emerald-500 focus:border-emerald-500" placeholder="{{ __('income.search_currency') }}">
                                        </div>
                                        <ul class="max-h-48 overflow-y-auto custom-scrollbar">
                                            <template x-for="cur in currenciesList.filter(c => c.type.toLowerCase().includes(searchCur.toLowerCase()))" :key="cur.id">
                                                <li @click="form.currency_id = cur.id; searchCurOpen = false; searchCur = ''" 
                                                    class="py-2.5 px-4 text-xs hover:bg-emerald-50 cursor-pointer font-bold border-b border-slate-50 last:border-0" 
                                                    :class="form.currency_id == cur.id ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700'"
                                                    x-text="cur.type">
                                                </li>
                                            </template>
                                        </ul>
                            ...
                            <div class="flex items-center gap-2">
                                <label class="text-xs font-bold text-emerald-800">{{ __('income.exchange_rate') }}</label>
                                <input type="number" step="0.000001" name="exchange_rate" x-model="form.exchange_rate" class="w-28 rounded-xl border border-white shadow-sm focus:ring-emerald-500 focus:border-emerald-500 text-sm text-center py-2 font-mono bg-white">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 p-5 rounded-2xl mb-6 grid grid-cols-1 md:grid-cols-3 gap-5 items-end shadow-inner mt-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.date') }} *</label>
                            <input type="date" name="income_date" x-model="form.date" class="w-full text-center rounded-xl border-slate-300 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-bold bg-white shadow-sm py-2.5" required>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.manual_voucher') }}</label>
                            <input type="text" name="manual_voucher" x-model="form.manual_voucher" class="w-full text-center rounded-xl border-slate-300 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-bold bg-white shadow-sm py-2.5" placeholder="---">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.attachment') }}</label>
                            <input type="file" name="attachment" class="w-full text-sm text-slate-500 file:mr-2 file:py-1.5 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer border border-slate-300 rounded-xl bg-white shadow-sm h-[42px] leading-[42px]">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-5 items-end mt-auto pt-2">
                        <div class="flex-1 w-full">
                            <label class="block text-xs font-bold text-slate-700 mb-1.5">{{ __('income.general_note') }}</label>
                            <input type="text" name="note" x-model="form.note" class="w-full text-sm rounded-xl border-slate-200 focus:ring-emerald-500 focus:border-emerald-500 shadow-sm bg-slate-50 hover:bg-white transition-colors py-3 px-4" placeholder="{{ __('income.general_note') }}...">
                        </div>
                        ...
                        <div class="flex items-center gap-3 shrink-0">
                            <span x-show="remainingAmount !== 0" class="text-xs font-bold text-red-600 animate-pulse bg-red-50 px-4 py-2 rounded-xl border border-red-200" dir="ltr">
                                {{ __('income.balance_must_be_zero') }}
                            </span>
                            <button type="submit" class="px-8 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-md hover:shadow-lg hover:shadow-emerald-500/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-base flex items-center justify-center gap-2" :disabled="remainingAmount !== 0 || (form.debt_amount > 0 && !form.account_id)">
                ...
                <div class="lg:col-span-1 p-6 bg-white space-y-6 rounded-2xl border border-slate-200 shadow-sm h-fit relative overflow-hidden">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.discount') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.cash') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-1.5">{{ __('income.debt') }}</label>
                            <input type="number" step="0.01" name="debt_amount" x-model.number="form.debt_amount" class="w-full text-center font-black text-blue-600 text-lg rounded-xl border-slate-200 focus:ring-blue-500 focus:border-blue-500 py-2.5 shadow-sm bg-white">
                        </div>
                    </div>
                    ...
                    <div x-data="{ searchOpen: false, searchAcc: '' }" class="relative">
                        <label class="block text-sm font-black text-slate-800 mb-2">{{ __('income.account') }}</label>
                        ...
                        <div class="relative" :class="form.debt_amount <= 0 ? 'opacity-60 grayscale cursor-not-allowed' : ''">
                            ...
                            <div x-show="form.debt_amount <= 0" class="absolute inset-0 z-10" title="{{ __('income.enter_debt_first') }}"></div>

                            <div @click="if(form.debt_amount > 0) { searchOpen = !searchOpen; if(searchOpen) $nextTick(() => $refs.searchInput.focus()) }" 
                                 class="w-full flex items-center justify-between border-slate-200 rounded-xl focus:ring-emerald-500 focus:border-emerald-500 text-sm bg-white shadow-sm py-3 px-4 font-bold transition-colors"
                                 :class="form.debt_amount > 0 ? 'cursor-pointer border hover:border-emerald-300' : 'border border-dashed bg-slate-50'">
                                <span x-text="form.account_id ? accountsData.find(a => a.id == form.account_id)?.name : '{{ __('income.select') }}'" :class="form.account_id ? 'text-emerald-700' : 'text-slate-400'"></span>
                                <svg class="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                            ...
                            <div x-show="searchOpen" @click.outside="searchOpen = false" x-transition.opacity class="absolute z-50 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden" style="display: none;">
                                <div class="p-2 border-b border-slate-100 bg-slate-50">
                                    <input x-ref="searchInput" type="text" x-model="searchAcc" class="w-full text-xs rounded-lg border-slate-300 py-2 px-3 focus:ring-emerald-500 focus:border-emerald-500" placeholder="{{ __('income.search_account') }}">
                                </div>
                                <ul class="max-h-56 overflow-y-auto custom-scrollbar">
                                    <li @click="form.account_id = ''; searchOpen = false; searchAcc = ''" class="py-2.5 px-4 text-xs hover:bg-slate-100 cursor-pointer text-slate-500">{{ __('income.select') }}</li>
                                    <template x-for="acc in accountsData.filter(a => a.name.toLowerCase().includes(searchAcc.toLowerCase()))" :key="acc.id">
                                        <li @click="form.account_id = acc.id; searchOpen = false; searchAcc = ''" 
                                            class="py-2.5 px-4 text-xs hover:bg-emerald-50 cursor-pointer font-bold border-b border-slate-50 last:border-0" 
                                            :class="form.account_id == acc.id ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700'"
                                            x-text="acc.name">
                                        </li>
                                    </template>
                                </ul>
                        ...
                        <p x-show="form.debt_amount <= 0" class="text-[10px] font-bold text-rose-500 mt-2">{{ __('income.enter_debt_first') }}</p>

                        {{-- INTERACTIVE ACCOUNT BALANCES CARD --}}
                        <div x-show="form.account_id && form.debt_amount > 0" class="mt-4 bg-emerald-50/60 rounded-2xl p-4 border border-emerald-100" x-transition>
                            <h4 class="text-xs font-bold text-emerald-800 mb-3 flex items-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path></svg>
                                {{ __('income.account_balances') }}
                            </h4>
                            <div class="space-y-2">
                                <template x-for="curId in getSelectedAccountCurrencies()" :key="curId">
                                    <div @click="form.currency_id = curId" 
                                         ...
                                         :class="form.currency_id == curId ? 'border-emerald-500 bg-white ring-1 ring-emerald-500 shadow-md transform scale-[1.02]' : 'border-emerald-100 bg-white/70 shadow-sm hover:border-emerald-300 hover:bg-white'">
                                        
                                        <div class="flex items-center gap-2.5">
                                            <div class="w-4 h-4 rounded-full border flex items-center justify-center transition-colors"
                                                 :class="form.currency_id == curId ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300 bg-white'">
                                                <div x-show="form.currency_id == curId" class="w-1.5 h-1.5 bg-white rounded-full"></div>
                                            </div>
                                            <span class="text-xs font-bold" :class="form.currency_id == curId ? 'text-emerald-800' : 'text-slate-500'" x-text="currenciesData[curId].type"></span>
                                        </div>
                                        
                                        <span class="text-sm font-black" 
                                              :class="getAccountBalance(curId) < 0 ? 'text-red-600' : (form.currency_id == curId ? 'text-emerald-800' : 'text-emerald-600')" 
                                              dir="ltr" 
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center bg-slate-50 py-3 px-4 rounded-xl border border-slate-100 shadow-sm">
                            <span class="text-xs font-bold text-slate-500">{{ __('income.total_income') }}</span>
                            <span class="text-xl font-black text-slate-800" x-text="totalIncome.toLocaleString()"></span>
                        </div>
                        <div class="flex justify-between items-center py-3 px-4 rounded-xl border shadow-sm transition-colors" :class="remainingAmount == 0 ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50'">
                            <span class="text-xs font-bold" :class="remainingAmount == 0 ? 'text-emerald-700' : 'text-red-700'">{{ __('income.remaining_amount') }}</span>
                    ...
                    <div x-show="form.cash_amount > 0" x-transition.opacity class="bg-emerald-50 p-4 mt-6 rounded-2xl border border-emerald-200 shadow-sm relative overflow-hidden">
                        ...
                        <label class="block text-xs font-bold text-emerald-800 mb-2">{{ __('income.cashbox') }} *</label>
