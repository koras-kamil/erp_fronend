<x-app-layout>
    ...
    <div x-data="{ 
        ...
        allIds: {{ json_encode($groups->pluck('id')->toArray()) }},
        ...
        // BULK FORCE DELETE (Using Custom Alert)
        bulkForceDelete() {
            if (this.selectedIds.length === 0) return;
            ...
            if (window.confirmAction) {
                ...
                // Fallback if custom script missing
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex items-center gap-3">
                ...
                <h3 x-show="selectedIds.length === 0" class="text-xl font-black text-slate-800 tracking-tight">{{ __('profit.menu_groups') }} ({{ __('profit.trash') ?? 'Trash' }})</h3>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="bg-white hover:bg-red-50/30 transition-colors group" :class="selectedIds.includes({{ $group->id }}) ? 'bg-red-50/60' : ''">
                        ...
                        <td class="px-4 py-4 text-center">
                            {{-- Important: :value="{{ $group->id }}" binds as integer to match allIds --}}
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                {{-- Single Restore --}}
                                <form action="{{ route('profit.groups.restore', $group->id) }}" method="POST">
                                    @csrf 
                                ...
                                {{-- Single Delete --}}
                                <button onclick="confirmDelete('{{ route('profit.groups.force-delete', $group->id) }}')" class="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors shadow-sm">
    ...
    <form id="bulk-restore-form" action="{{ route('profit.groups.bulk-restore') }}" method="POST" class="hidden">
        @csrf 
        <input type="hidden" name="ids" id="bulk-restore-ids">
    </form>

    <form id="bulk-force-delete-form" action="{{ route('profit.groups.bulk-force-delete') }}" method="POST" class="hidden">
        @csrf 
        @method('DELETE')
