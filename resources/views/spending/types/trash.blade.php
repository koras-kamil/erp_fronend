<x-app-layout>
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 px-4 no-print">
            ...
            <div class="flex items-center gap-3">
                ...
                <h3 x-show="selectedIds.length === 0" class="text-xl font-black text-slate-800 tracking-tight">{{ __('spending.type_header') }} ({{ __('spending.trash') ?? 'Trash' }})</h3>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-slate-500">
                <thead class="text-xs text-slate-700 uppercase bg-red-50/50 border-b border-red-100">
                    <tr>
                        ...
                        {{-- REMOVED ACCOUNTANT CODE COLUMN --}}
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="bg-white hover:bg-red-50/30 transition-colors group" :class="selectedIds.includes({{ $type->id }}) ? 'bg-red-50/60' : ''">
                        ...
                        {{-- REMOVED ACCOUNTANT CODE DATA CELL --}}
    ...
    <form id="bulk-restore-form" action="{{ route('type-spending.bulk-restore') }}" method="POST" class="hidden">@csrf <input type="hidden" name="ids" id="bulk-restore-ids"></form>
    <form id="bulk-force-delete-form" action="{{ route('type-spending.bulk-force-delete') }}" method="POST" class="hidden">@csrf @method('DELETE') <input type="hidden" name="ids" id="bulk-force-delete-ids"></form>
    ...
    <script>
        function confirmForceDelete(id) {
            ...
            form.action = "{{ route('type-spending.force-delete', ':id') }}".replace(':id', id);
