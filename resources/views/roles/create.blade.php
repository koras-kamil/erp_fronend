<x-app-layout>
    <div class="py-12 bg-[#f8fafc] min-h-screen pb-20">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-2xl font-black text-slate-800 tracking-tight">{{ __('roles.title') }}</h1>
                    <p class="text-xs text-slate-400 font-medium">{{ __('roles.subtitle') }}</p>
                </div>
                <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-indigo-600 transition-all group bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
            ...
            <form action="{{ route('roles.store') }}" method="POST">
                ...
                <div class="bg-white rounded-[1.5rem] p-6 shadow-sm border border-slate-200/60 mb-6">
                                ...
                                <tr class="group hover:bg-indigo-50/30 transition-all">
                                    <td class="p-4 px-6 align-middle">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-black text-slate-700 text-sm uppercase tracking-tight">
                                                {{ __('roles.' . $module) }}
                                            </span>
                                        </div>
                                    </td>
                                        ...
                                        <div class="flex flex-wrap items-center gap-3 justify-end">
                                            ...
                                            <label class="inline-flex items-center gap-2 p-2 px-3 rounded-lg border border-slate-100 bg-white hover:border-indigo-200 cursor-pointer shadow-sm transition-all group/item">
                                                ...
                                                <span class="text-[11px] font-bold text-slate-500 group-hover/item:text-slate-800 capitalize">
                                                    @php $action = trim(str_replace(['roles.', $module, 'es', 's', '-', '.', ' '], '', strtolower($permission->name))); @endphp
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit" 
                            class="w-full md:w-auto min-w-[240px] relative group overflow-hidden bg-[#0f172a] hover:bg-indigo-600 text-white font-black py-3.5 px-8 rounded-xl shadow-lg transition-all duration-300 active:scale-[0.98] flex items-center justify-center gap-3">
                        <div class="absolute inset-0 w-1/2 h-full bg-white/5 skew-x-[-25deg] -translate-x-full group-hover:animate-[shimmer_2s_infinite]"></div>
                        <span class="relative uppercase tracking-widest text-xs font-black">{{ __('roles.btn_create') }}</span>
                        <svg class="w-4 h-4 relative transition-transform group-hover:translate-x-1 rtl:group-hover:-translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg>
                    </button>
                </div>
            </form>
        </div>
    ...
    <style>
        @keyframes shimmer { 100% { transform: translateX(400%) skewX(-25deg); } }
    </style>
</x-app-layout>
